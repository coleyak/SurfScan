...
class SurfacePoint:
    ...
    def residual(self) -> float:
        """Raw in-sample residual: actual - fitted. Kept for diagnostics
        (e.g. fit_quality's R^2), but detection should prefer loo_residual --
        see the module docstring for why in-sample residuals understate
        how anomalous a high-leverage point really is."""
        return self.actual_iv - self.fitted_iv

    @property
    def loo_residual(self) -> float:
        """Leverage-corrected residual: an approximation of the residual
        this point would have had if it hadn't influenced the fit at all."""
        return self.residual / (1.0 - self.leverage)


class VolatilitySurface:
    """A smoothed implied-volatility surface fit to one option type of one chain."""
    ...
    def fit(
        ...
        """Fit the quadratic surface with Huber-weighted IRLS.

        huber_delta is expressed in IV units (e.g. 0.03 = 3 vol points).
        Residuals smaller than huber_delta are treated as normal noise
        (weight 1); larger residuals are down-weighted so a handful of
        outliers can't pull the fitted surface toward themselves.
        """
        ...
        self._leverage = self._compute_leverage(A, weights)
        return self

    @staticmethod
    def _compute_leverage(A: np.ndarray, weights: np.ndarray) -> np.ndarray:
        """Diagonal of the weighted hat matrix: h_ii = w_i * a_i^T (A^T W A)^-1 a_i."""
        AtWA = A.T @ (weights[:, None] * A)
        AtWA_inv = np.linalg.pinv(AtWA)
        M = A @ AtWA_inv  # n x p
        h = weights * np.einsum("ij,ij->i", M, A)
        return np.clip(h, 0.0, 0.99)
    ...
    def points(self) -> List[SurfacePoint]:
        ...
        return [
            SurfacePoint(contract=c, x=x, y=y, actual_iv=z, fitted_iv=f, leverage=lev)
            for c, x, y, z, f, lev in zip(
                self._contracts, self._x, self._y, self._z, fitted, self._leverage
            )
        ]

    def fit_quality(self) -> dict:
        """Simple diagnostics: R^2 and RMSE of the fit against raw (unweighted) data."""
        pts = self.points()
